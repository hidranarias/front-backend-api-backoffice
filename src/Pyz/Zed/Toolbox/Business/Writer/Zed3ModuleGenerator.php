...
class ZedModuleGenerator implements ModuleGeneratorInterface
{
    ...
    public function generateModuleFromConfig(string $configFilePath = '', string $content = ''): void
    {
        ...
        if (isset($config['views'])) {
            $this->generateViews($moduleBasePath, $config);
        }
...
PHP;
    ...
    private function generateViews(
        ...
        foreach ($config['views'] as $name => $files) {
            $viewPath = sprintf('%s/Presentation/%s', $moduleBasePath, $name);
            if (!is_dir($viewPath) && !mkdir($viewPath, 0775, true) && !is_dir($viewPath)) {
                throw new RuntimeException(sprintf('Directory "%s" was not created', $viewPath));
            }
            foreach ($files as $file) {
                $filePath = sprintf('%s/%s.php', $viewPath, $file);
                if (!file_put_contents($filePath, '<!-- -->')) {
                    throw new RuntimeException("Failed to create view file at: $filePath ");
                }
            }
        }
    }
